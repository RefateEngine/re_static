...
# 空いてる場合終了
execute if block ~ ~ ~ #minecraft:doors[open=true] run return fail

# half=upperの場合lowerへ移動して実行
execute if block ~ ~ ~ #minecraft:doors[half=upper] positioned ~ ~-1 ~ run return run function re_static:update/door/open
...
# open
data modify storage re_static: update.door.open set value "true"
...
# sound
function re_static:update/door/z/sound/open